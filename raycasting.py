        ...
        for i in range(len(self.game.map.world_map)):
            for ray, values in enumerate(self.ray_casting_result[floor]):
                ...
                wall_column = pygame.transform.scale(wall_column,
                                                    (SCALE, abs(proj_height)))
                wall_pos = (ray * SCALE, (HALF_HEIGHT - proj_height // 1.4- (HALF_HEIGHT  // 4) - (floor * proj_height)-self.game.player.vert_angle) + (self.game.player.z / depth) +300)
